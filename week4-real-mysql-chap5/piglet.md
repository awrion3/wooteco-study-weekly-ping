...
- **READ UNCOMMITTED**에서는 트랜잭션 A가 데이터를 수정했지만 아직 커밋하지 않아도 트랜잭션 B에서 해당 데이터를 읽을 수 있음.
    - 즉, **Dirty Read**가 발생하는 수준.
- 이를 방지하려면 **READ COMMITTED 이상**의 격리 수준을 사용해야 함.
    - **READ COMMITTED**부터는 트랜잭션이 커밋되기 전까지 다른 트랜잭션에서는 변경된 데이터를 볼 수 없음.
...
- 문제 상황 요약
    1. **트랜잭션 A:** `SELECT * FROM users WHERE id = 1;` 실행
    2. **트랜잭션 B:** `id = 1` 데이터를 수정 후 커밋
    3. **트랜잭션 A:** 동일한 `SELECT` 실행 시 **다른 값 조회됨** → **NON-REPEATABLE READ 발생**

- **NON-REPEATABLE READ를 방지하려면?**
    - **REPEATABLE READ 이상의 격리 수준이 필요함.**
    - **READ COMMITTED에서는 NON-REPEATABLE READ 발생 가능**
        - 트랜잭션 A가 첫 번째 `SELECT` 실행 후, 트랜잭션 B가 데이터를 수정하고 커밋하면 A가 다시 조회할 때 수정된 값이 반영됨.
    - **REPEATABLE READ에서는 방지됨**
        - 트랜잭션 A가 처음 `SELECT`를 실행하면 **트랜잭션이 끝날 때까지 동일한 결과를 조회**하도록 보장됨.
        - **MVCC를 사용해 Undo 영역에서 기존 데이터 버전을 조회**하므로 같은 결과가 반환됨.
...
- 트랜잭션이 시작되면 **"스냅샷(Snapshot)"을 생성**하고, 같은 트랜잭션 내에서는 동일한 데이터 버전을 유지함.
- 다른 트랜잭션에서 변경을 하더라도 **해당 트랜잭션이 시작될 때 보였던 데이터를 계속 조회**하게 됨.
- 이를 위해 **Undo 영역에 백업된 이전 데이터를 활용**하여 동일한 값을 반환함.

1. 데이터 변경이 발생하면, 기존 데이터를 Undo 영역에 백업
2. REPEATABLE READ에서는 트랜잭션이 시작될 때의 스냅샷을 기준으로 데이터 조회
    - 트랜잭션 A가 `SELECT * FROM users WHERE id = 1;`을 실행한 이후,
    - 트랜잭션 B에서 `UPDATE users SET name = 'newName' WHERE id = 1;` 후 커밋
    - 트랜잭션 A가 다시 `SELECT`를 실행하면 Undo 영역에서 **처음 조회했던 값**(`'oldName'`)을 반환
3. 트랜잭션이 끝난 후에는 최신 데이터가 반영됨
    - 트랜잭션 A가 종료되면 이후 실행되는 `SELECT`는 `'newName'`을 조회
